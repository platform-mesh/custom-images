...
# Read the prefix from environment variable for subpath deployments
# e.g., HTTPBIN_PREFIX="/httpbin" for serving at localhost/httpbin/
...
# Configure APPLICATION_ROOT if a prefix is set - this makes url_for() generate correct URLs
if HTTPBIN_PREFIX:
    app.config["APPLICATION_ROOT"] = HTTPBIN_PREFIX

app.add_template_global("HTTPBIN_TRACKING" in os.environ, name="tracking_enabled")
# Inject the prefix into templates for use in base tag and other prefix-aware elements
app.add_template_global(HTTPBIN_PREFIX, name="httpbin_prefix")
...
class PrefixMiddleware:
    """
    WSGI middleware that sets SCRIPT_NAME from the HTTPBIN_PREFIX environment variable.
    This allows httpbin to be served behind a reverse proxy at a subpath.

    For example, if HTTPBIN_PREFIX="/httpbin", requests to /httpbin/get will be
    properly routed to the /get endpoint, and url_for() will generate URLs
    with the /httpbin prefix.

    Requests that don't start with the prefix will receive a 404 response.
    """
    ...
    def __call__(self, environ, start_response):
        ...
        # Only serve requests that start with the prefix
        if not path_info.startswith(self.prefix):
            # Return 404 for requests outside the prefix
            status = '404 Not Found'
            response_headers = [('Content-Type', 'text/plain')]
            start_response(status, response_headers)
            return [b'Not Found']

        # Set SCRIPT_NAME to the prefix so Flask generates correct URLs
        environ['SCRIPT_NAME'] = self.prefix
        # Strip the prefix from PATH_INFO for routing
        environ['PATH_INFO'] = path_info[len(self.prefix):] or '/'
        return self.app(environ, start_response)


# Apply prefix middleware if HTTPBIN_PREFIX is set
if HTTPBIN_PREFIX:
